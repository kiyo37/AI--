...
| 使うファイル | [業務分析シート](templates/business_analysis_sheet.md)、[用語集](glossary.md)、AIを使う場合だけ[対話AIアカウント準備](setup/chat_ai_account_setup.md) |
| 個人学習 | 教材の架空業務を使います。自分の業務は名称と工程だけを一般化し、実データをAIへ入力しません |
| 組織利用 | 承認済みAI・業務用アカウント・保存先だけを使い、実データによる試行はこの教材と分離します |
| AIなし | 付箋、紙、Markdown、業務分析シートだけで動作を分解し、AI候補は自分で仮置きします |
| 成果物 | 業務の境界、1行1動作のBefore、AI・通常自動化・人のAfter、効果指標、停止条件を含む業務分析シート |
| 開始条件 | 完全な架空ケース、保存場所、相談先を確認し、業務の開始と完了を1文ずつ書けること |
| 停止条件 | 実在データが必要、業務の責任者が不明、AIへ最終判断や外部送信を任せる案になった場合は止めて紙上設計へ戻ること |
...
- ChatGPTの提案を業務分析シートへ転記し、人間が採用・修正・却下を判断する練習
...
### 演習：ChatGPTと業務分析シートで月次問い合わせ報告を分解する

架空のサポート部では、毎月300件の問い合わせを担当者が分類し、上位5テーマと改善案を報告しています。実在する顧客・従業員・案件の情報は使いません。

使用物は[業務分析シート](templates/business_analysis_sheet.md)、筆記用具、利用を許可されたChatGPTです。ChatGPTの画面や機能名は変わるため、当日は最新の公式情報と組織のルールを優先します。

#### 利用環境の準備確認

開始前に[AIツール共通準備](setup/common_preparation.md)と[対話AIアカウント準備](setup/chat_ai_account_setup.md)を確認します。ChatGPTの利用は必須ではなく、紙上だけでも同じ完成条件まで進められます。

| 学習環境 | 実施方法 |
|---|---|
| 個人学習 | 自分で利用条件とデータ設定を確認できるアカウントだけを使います。完全な架空データだけを入力し、利用したくない場合は紙上代替を選びます。 |
| 組織利用 | 承認済みアカウント、入力ルール、保存・共有設定を確認します。承認済み環境でも、入力は教材に記載された完全な架空データだけに限定します。 |
| AIなし | 付箋または業務分析シート上で、自分がAI候補を仮置きし、採用・修正・却下を判断します。 |

開始前に、画面共有へ通知や別ファイル名が映らないことも確認します。未承認の環境しかない場合は入力せず、紙上代替へ切り替えます。

#### 手順付き練習

まず「問い合わせメールを受け取り、担当部署へ振り分ける」という小さな業務を、開始・入力・動作・判断・出力・例外・完了に分けてシートへ記入します。その後、次のプロンプトをChatGPTへ入力します。
...
- ChatGPTには架空業務の条件だけを渡し、分解漏れと確認質問の候補を出させる
...
次の順でシートとChatGPT出力を自分で照合します。学習仲間がいる場合の相互確認は任意です。
...
- ChatGPTが見つけた有用な観点と、人間が修正した点をそれぞれ記録する
