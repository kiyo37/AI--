...
- ブラウザでリンクを開ける
- 本人または組織が利用を認めた対話AIを、少なくとも一つ実際に操作できる
- 個人学習では自分の利用環境を、組織利用では承認済み環境と正式窓口を確認できる
- 架空データと実在データを混ぜないと約束できる

有料契約、新規アカウント、Office製品、特定のAI製品は必須ではありません。ただし、対話AIの実操作は必須です。
...
どちらも、`sample-data/`の完全な架空データだけをAIへ入力します。個人学習でも、自分の仕事のデータへ置き換えません。利用できる対話AIがない場合は演習を開始せず、[セットアップガイド](setup/README.md)で環境を準備します。
...
5. 分からない場合は入力前に止め、個人学習では公式情報を確認し、組織利用では正式窓口へ相談する
...
2. 個人学習または組織利用のどちらかを選び、対話AIへログインできることを確認する
3. 説明と手順付き練習を読む
...
第08・09章でもAIの実操作を行います。第08章はNotebookLMまたは承認済みの同等RAG機能が必要で、どちらも利用できない場合はこの任意章を飛ばします。第09章は承認済み対話AIで架空データを分類し、自動化製品の実機部分だけを飛ばせます。保存済み出力は比較・誤り分析の補助であり、それだけでは章の完了になりません。APIキーの設定や外部システムへの書込みは基本経路で行いません。
...
| 状況 | 次に行うこと |
|---|---|
| AIへログインできない | 演習を開始・継続せず、[対話AIアカウント準備](setup/chat_ai_account_setup.md)へ戻る。組織利用では正式窓口へ確認する |
