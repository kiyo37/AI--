...
次の架空例を「入力可」「条件付き」「入力不可／相談」に分類し、理由、代替方法、承認者を書きます。

1. 公開済み自社Webページの文章
2. 氏名・電話番号を含む顧客苦情メール
3. 架空の顧客名へ置換したメールテンプレート
4. 未公開の価格改定表
5. 画面に表示されたAPIキー
6. 参加者へ事前通知していない会議音声

製品名だけで判断せず、データ分類、契約、設定、目的、最小化、出力管理を記入してください。

### 演習2：誤入力時の初動（20分）

架空の担当者が、エラー画面のスクリーンショットを承認済みAIへ入力した直後、画面内にAPIキーが表示されていたことに気付きました。

1. 最初の5分で行うことを順番に並べます。
2. AIの会話削除だけでは終えない理由を説明します。
3. 誰へ、どの事実を、どの連絡手段で報告するかを書きます。
4. APIキーの失効・交換、アクセスログ確認、影響評価の担当を割り当てます。
5. 再発防止を「画面」「手順」「教育」の3面で提案します。

**評価基準**：隠蔽せず、停止・記録・報告を先に行い、権限ある担当者による失効・影響確認へつないでいること。

## 7. 確認問題

1. 氏名を削除すれば必ず安全と言えない理由は何ですか。
2. 法人向けAIでも確認すべき事項を4つ挙げてください。
3. APIキーを誤入力したとき、会話削除だけでは不十分な理由は何ですか。
4. シャドーAIを禁止通知だけで解決しにくい理由は何ですか。
5. 企業研修の開始時に確認すべき社内情報は何ですか。

<details><summary>解答例</summary>

1. 部署、役職、案件、日時などとの組み合わせで個人を再識別できる場合があるためです。
2. データ利用、保存・削除、権限、監査ログ、処理地域、契約などです。
3. 秘密情報を取得された可能性が残るため、失効・交換と影響確認が必要です。
4. 現場の必要性と安全な代替手段がなければ、利用が見えない場所へ移るためです。
5. 承認済みツール、入力禁止情報、データ分類、相談・事故報告先などです。
