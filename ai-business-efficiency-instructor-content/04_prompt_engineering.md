...
| 個人学習 | 本人が管理し、利用条件とデータ設定を確認した対話AIを使います。入力は教材の架空会議メモだけにします |
| 組織利用 | 組織が承認したURL・業務用アカウント・保存先だけを使います。未確認ならAIを開きません |
| 成果物 | 6要素を説明したメモ、1点だけ改善した追加指示、改善後の議事録・タスク一覧、再照合記録 |
| 開始条件 | 承認・利用条件を確認した対話AIを操作でき、元資料、変更前の確認済み成果物、変更する1条件、保存場所がそろっていること |
| 停止条件 | 対話AIを利用できない、実在情報を入力しそう、複数条件を同時に変更した、元資料と照合できない、外部共有・送信が必要になった場合は止めること |

> **修了条件**：プロンプトを読むだけではなく、基準指示と改善指示を対話AIへ送り、出力差を照合する**AIの実操作**が必要です。AIを利用できない場合は、環境を準備できるまで演習を停止します。
...
- 対話AIを利用できない場合は演習を開始せず、[対話AIアカウント準備ガイド](setup/chat_ai_account_setup.md)で環境を整えてから戻ります。[保存済みの問題出力](sample-outputs/beginner_minutes_saved_output.md)は誤りの観察に使えますが、プロンプト送信の代わりにはなりません。外部共有・送信は行いません。
...
| 対話AIを利用できない | 演習を停止し、承認・利用条件を確認した対話AIを準備してから戻る。保存済み成果物の閲覧だけでは完了にしない |
...
2. 基準出力を作った同じAI会話へ追加指示を1回送ります。
3. 元の確認済み成果物は比較用に残し、AIが生成した改善版を別の成果物として保存します。
