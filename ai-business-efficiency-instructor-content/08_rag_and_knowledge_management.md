...
| 開始条件 | 下の2経路から1つを選び、NotebookLMまたは承認・利用条件を確認した同等のRAG機能と、4つの収録ファイルを利用できる |
| 停止条件 | RAG機能を利用できない、実在する社内文書を追加しそう、閲覧権限が不明、現行版を特定できない、資料外の回答を断定している |

### 利用環境を1つ選ぶ

| 経路 | 進め方 |
|---|---|
| 個人学習 | 本人が利用条件とデータ設定を確認したNotebookLMまたは同等のRAG機能を使い、収録済みの架空規程だけを追加する |
| 組織利用 | 組織が承認した業務アカウント、RAG機能、ブラウザだけを使い、収録済みの架空規程だけを追加する |

> **修了条件**：架空規程をRAG機能へ追加し、資料内・資料外・版競合の質問を実行して根拠を照合する**AIの実操作**が必要です。利用できる承認済みRAG機能がない場合、この任意発展章は飛ばすか、環境を準備してから再開します。資料カードと保存済み回答は理解補助・誤り分析に限り、実操作の代わりにはしません。
...
機能や利用条件は変わるため、[NotebookLMの準備ガイド](setup/notebooklm_setup.md)、[公式ヘルプ](https://support.google.com/notebooklm/answer/16164461)と組織の契約・管理設定を確認します。組織利用では管理者が許可した業務アカウントだけを使い、完全な架空資料だけを追加します。NotebookLMを利用できない場合は、承認・利用条件を確認した同等のRAG機能を使います。どちらも利用できない場合は、保存済み回答を読むだけで完了にせず、この任意発展章を飛ばすか環境を準備してから再開します。
...
Difyは発展学習の選択肢であり、本章の修了条件にはしません。実行には、Dify内でAIモデルを呼び出すための利用枠である**AIクレジット**、またはAIモデルを提供する外部サービスである**モデル提供者**の設定・認証情報が必要になる場合があります。APIキーをチャット欄、教材、画面共有へ貼り付けてはいけません。利用前に[Difyの任意発展準備](setup/dify_setup_optional.md)と[公式ドキュメント](https://docs.dify.ai/en/home)を確認します。承認済み環境や認証情報がない場合はDify実機練習を飛ばせます。紙上設計は仕組みの理解補助には使えますが、Difyの操作経験とは記録しません。
...
[現行版の架空研修運営規程](sample-data/rag_training_rules_current.md)と[旧版](sample-data/rag_training_rules_old.md)を使い、資料にある質問・ない質問・版が競合する質問をRAG機能で試します。質問設計には[RAG質問・資料カード](sample-data/rag_question_cards.md)、自分の出力の誤り分析には[NotebookLM演習の保存済み回答](sample-outputs/notebooklm_saved_answers.md)を参考にできます。Difyは任意の追加練習であり、両方を使う必要はありません。

#### 1. 利用環境の準備確認

最初に[NotebookLMの準備ガイド](setup/notebooklm_setup.md)を開き、次のどちらで進めるか決めます。

| 状況 | 進め方 |
|---|---|
| 個人学習 | 本人が管理し、利用条件とデータ設定を確認したNotebookLMまたは同等のRAG機能を使い、教材の完全な架空資料だけを追加する |
| 組織利用 | 管理者が許可した業務アカウント・RAG機能・ブラウザを使う。不明なら登録・アップロードせず、本章を飛ばすか環境を準備する |
...
2. NotebookLMまたは承認済みの同等機能へ両方の規程を追加し、「申込期限は何ですか。根拠文書と該当箇所を示してください」と質問します。
3. 質問に関係する資料カードを自分でも選び、AI回答・根拠・注意・相談先と比較します。
...
- Difyを試す場合だけ、[Difyの任意発展準備](setup/dify_setup_optional.md)に従います。承認済み環境やモデル設定がない場合はDify実機練習を飛ばし、必要なら検索、回答、権限、人間確認の箱を紙へ描いて設計理解を補います。この紙上記録はDifyの操作経験には数えません。
...
- [ ] 必須のRAG実機練習と、任意のDify実機練習、理解補助の資料カードを区別して説明できる
