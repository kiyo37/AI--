...
本章の基本ハンズオンにはNotebookLMを使います。NotebookLMは、受講者が指定した資料を基に質問し、回答と引用箇所を照合する流れを体験しやすいツールです。ただし、引用が表示されても回答の正しさが保証されるわけではありません。

機能や利用条件は変わるため、[NotebookLMの準備ガイド](setup/notebooklm_setup.md)、[公式ヘルプ](https://support.google.com/notebooklm/answer/16164461)と組織の契約・管理設定を確認します。企業研修では管理者が許可した業務アカウントだけを使い、個人学習では完全な架空資料または利用権を確認した公開資料だけを使います。未承認・利用可否不明・登録できない場合は、配布した資料カードと回答例を使う紙上演習へ切り替えます。
...
**講師向け説明ポイント**：回答の正確さだけでなく、「資料外で止まれたか」と「権限違反が起きなかったか」を別の観察項目として扱います。本番後も文書や質問傾向が変わるため、この循環を継続する必要があると説明します。
...
架空の「研修運営規程」を使い、資料にある質問・ない質問・版が競合する質問を試します。基本演習はNotebookLMです。Difyは任意の発展演習であり、両方を使う必要はありません。
...
| 企業研修 | 管理者が許可した業務アカウント・機能・ブラウザを使う。講師判断で個人登録させない |
| 未承認・利用不可 | 登録やアップロードをせず、配布した文書カードと保存済み回答例で紙上演習を行う |

実在の社内文書、個人情報、機密情報、認証情報は使いません。

#### 2. ガイド付き練習

1. 講師が配布した架空規程へ、文書ID、版、所有者、対象、施行日を付けます。
...
この節は採点しません。自分が説明・実演できるかを確認し、不明な項目を次の学習課題にします。
