...
| 企業研修 | 研修担当者が承認済みアカウント、入力ルール、保存・共有設定を事前確認します。受講者へ個人アカウントの登録を強制しません。 |
| ChatGPTを利用できない場合 | 講師が用意した架空のAI出力例を配布し、付箋または業務分析シート上で同じ判断を行います。 |
...
- 完全な架空データで行うPoCの範囲、成功条件、停止条件、元へ戻す方法を決める

**停止条件例**：個人情報の混入、分類の重大誤り、根拠のない原因断定、承認前の外部送信。

#### 人間レビュー

別の受講者または講師が、次の順でシートとChatGPT出力を照合します。
...
| 観察ポイント | 受講者がツール名から始めず、実際の動作と例外を言葉にできるか。AIへ最終判断を移していないか |
| 改善ポイント | 粒度が粗い場合は「誰が、何を見て、何を変え、どこへ渡すか」で再分解する。効果だけの場合は確認時間と失敗時の損失を追加する |

## 7. 実務チェックリストと振り返り

この節は知識を採点するテストではありません。実務へ持ち帰る前に、判断過程を自分で点検するために使います。
...
- [ ] 完全な架空データまたは承認済みデータだけでPoCする設計にした
