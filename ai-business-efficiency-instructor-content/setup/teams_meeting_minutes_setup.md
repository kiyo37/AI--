...
> **基本学習ではTeamsを操作しません。** 教材に保存済みの完全な架空文字起こしを使うため、Teamsのアカウント、ライセンス、録画、実在参加者の音声は不要です。実際のTeams会議を使う操作は、組織の承認、管理者設定、主催権限、参加者への事前説明、必要な同意をすべて満たした場合だけ行う任意発展です。
...
| 読む人 | 先に読む範囲 | 次に行うこと |
|---|---|---|
| 基本演習を行う人 | 1〜4、10、13 | [保存済み架空文字起こしの演習](../exercises/video_meeting_minutes_exercises.md)を行う。実機操作はしない |
...
|---|---|---:|---:|---|
| A：保存済み教材（個人・組織・AIなし） | 架空文字起こし、照合資料、ワークシート | 不要 | 使用しない | **すべての人の基本演習** |
...
5. AIを使えない場合は、[保存済みの問題出力](../sample-outputs/video_meeting_ai_minutes_problem.md)をAIの初回出力としてレビューします。
...
6. **AIなし経路を確認する**：アカウントや機能がなくても同じ学習目標を達成できるよう、[保存済み教材](#13-teamsを使わない基本演習)を開きます。
