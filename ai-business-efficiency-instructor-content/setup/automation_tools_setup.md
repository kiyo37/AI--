...
このガイドは、[09 AI自動化とAIエージェント](../09_ai_automation_and_agents.md)の任意ハンズオンで使用するツールを一つ選び、安全に準備するためのものです。Power Automate、Make、Zapierのすべてへ登録する必要はありません。どの製品も利用できない場合はこの実機演習を飛ばしますが、第09章のコア対話AIによるAIの実操作は続けます。
...
```mermaid
flowchart TD
    A{"学習経路"} -->|個人学習| B["本人が規約・料金・設定を確認"]
    A -->|組織利用| C["組織の承認・権限・契約を確認"]
    B --> D{"安全な閉じたテストが可能か"}
    C --> D
    D -->|はい| E["一製品で架空テスト"]
    D -->|いいえ・不明| F["実機演習を停止・任意発展を飛ばす"]
    F --> G["コア対話AIの学習へ戻る"]
```
...
| 指定なし・未承認・利用不可 | 実機演習を行わない | コア対話AIを使う第09章の基本演習へ戻る |
...
| 確認項目 | 個人学習 | 組織利用 |
|---|---|---|
| 登録 | 本人が公式サイト、規約、料金を確認して判断 | 管理者が発行・招待した業務アカウントを使用 |
| メール | 本人が管理するメール | 組織指定の業務メール |
| 契約・試用 | 課金開始、更新、解約条件を本人が確認 | 購買・管理者が契約とライセンスを確認 |
| 接続先 | 架空データだけ。実在業務サービスへ接続しない | 外部サービスへ接続しない閉じたテスト環境。実在の宛先・ファイル・予定・データベースは使わない |
| 認証 | 強いパスワードと利用可能なMFA | 組織指定のSSO・MFA |
| 演習結果 | 画面内のテスト結果だけ | 承認済みテスト環境の履歴だけ |

組織利用では、自己サービス登録が画面に表示されても、管理者承認前に進めません。Power Automateでは組織が自己登録を無効にしている場合があります。その場合は個人メールへ切り替えず、IT管理者へ連絡し、準備が整うまでは実機演習を停止します。
...
- [ ] MFAまたは組織指定の認証を設定した
- [ ] 完全な架空データと、利用できない場合の停止・復帰手順を用意した
...
6. 組込み処理だけでは試せない場合や、外部接続を要求された場合は保存せず、実機演習を停止します。
...
ツールを利用できない場合や画面が変わっている場合は、復旧または承認済み代替ツールの準備まで実機演習を停止します。[自動化フローカード](../sample-data/automation_flow_cards.md)と[保存済みテスト出力](../sample-outputs/automation_saved_test_output.md)は誤り分析やトラブルの切り分けにだけ使用し、実機操作の代替にはしません。
...
1. 無断登録、個人アカウントへの切替、外部コネクタの追加を行わず、実機演習を停止します。
2. 個人学習では、本人が条件を確認できる別の自動化ツールがある場合だけ、完全な架空データで試します。
3. 組織利用では、管理者へ承認済みツールと閉じたテスト環境の有無を確認します。
4. 承認済み代替ツールがなければこの任意発展を飛ばし、第09章のコア対話AIを使う基本演習へ戻ります。
5. 利用環境を準備できた場合だけ、このガイドへ戻って実機演習を行います。

[保存済みテスト出力](../sample-outputs/automation_saved_test_output.md)の確認だけでは、フローの作成、テスト、履歴確認という実操作を経験できません。保存済み出力は補助資料であり、自動化ツールまたは承認済み代替ツールによる実機演習の完了条件にはしません。
