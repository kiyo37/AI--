...
Difyは、[08 RAGとナレッジマネジメント](../08_rag_and_knowledge_management.md)の任意発展演習です。先にNotebookLMまたは承認済みの同等RAG機能を使う第08章の基本演習を終えてください。Difyへの登録、APIキーの取得、モデル契約は必須ではありません。
...
| 利用可否不明・登録不可・認証情報なし | Difyの任意発展を停止し、第08章の承認済みRAG機能を使う基本演習へ戻る |

組織利用では、個人アカウント作成、クレジット購入、試用開始、外部モデル契約を学習条件にしません。個人学習でも、料金・契約・APIキーが必要な場合はDifyの任意発展を飛ばせます。Difyを飛ばしても、第08章では承認済みRAG機能によるAIの実操作を行います。
...
- キーや請求先の管理者が不明なら、Difyを実行せず任意発展を停止する
...
- [ ] 完全な架空FAQと、Difyを使えない場合の停止・復帰手順を用意した
...
Difyは任意発展です。利用条件を満たせない場合は、次のように対応します。

1. 無断登録、APIキーの追加、有料契約、設定の回避を行わず、Difyの操作を停止する
2. 組織が承認した同等のRAGツールがあれば、その正式手順で完全な架空資料だけを使う
3. 同等ツールがなければDifyの任意発展を飛ばし、第08章の承認済みRAG機能を使う基本演習へ戻る
4. Difyの利用環境を準備できた場合だけ、このガイドへ戻って実機演習を行う

[Dify設計の保存済み例](../sample-outputs/dify_saved_design_output.md)は、第08章のRAG実操作後に設計上の誤りを見つける追加分析としてだけ使用します。保存済み例の確認は、Difyまたは承認済み同等ツールでの実操作を完了したことにはなりません。
