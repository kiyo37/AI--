...
| 使うファイル | [用語集](glossary.md)、[AIツール共通準備](setup/common_preparation.md)、[対話AIアカウント準備](setup/chat_ai_account_setup.md) |
| 個人学習 | 本人が管理し、利用条件とデータ設定を確認した対話AIを使います。入力は教材の完全な架空ケースだけです |
| 組織利用 | 自組織の正式な情報分類・AI利用ルール・事故窓口を確認します。教材の分類例を社内規程の代わりにしません |
| 成果物 | 入力判断表、APIキー誤入力時の初動フロー、安全な代替方法、相談先メモ |
| 開始条件 | 承認・利用条件を確認した対話AIを操作でき、完全な架空ケースだけを使い、個人学習と組織利用のどちらで進めるか決めていること |
| 停止条件 | 対話AIを利用できない、本物かもしれない情報、認証情報、実在画面・音声・文書が出た場合は入力せず止めること。組織利用で規程・窓口が不明なら演習を開始しないこと |

> **修了条件**：この章では、紙やMarkdownで先に人の判断を記録したうえで、教材の架空ケースを承認済み対話AIへ入力する**AIの実操作**が必要です。AIを利用できない場合は、環境を準備できるまで演習を停止します。
...
### 演習：AI出力と照合する入力判断と誤入力対応

この演習では、紙またはMarkdownへ人の判断を先に記録し、その後に承認済み対話AIへ教材の完全な架空データだけを入力して比較します。実在するメール、スクリーンショット、名簿、会議音声、社内文書、認証情報は使いません。

#### 利用環境の準備確認

最初に[AIツール共通準備](setup/common_preparation.md)と[対話AIアカウント準備](setup/chat_ai_account_setup.md)で、停止・相談条件と利用環境を確認します。対話AIを利用できない場合は演習を停止し、環境を準備してから戻ります。

| 学習環境 | 実施方法 |
|---|---|
| 個人学習 | 本人が利用条件とデータ設定を確認した対話AIを使います。ケースカードを紙またはローカルのメモへ写して先に判断し、架空ケースをAIへ入力した結果と比較します。 |
| 組織利用 | 自組織の情報分類表、AI利用ルール、事故連絡先を確認します。承認済み環境でも教材に記載された完全な架空データだけを使います。 |

未承認の環境しかない場合は個人アカウント等へ切り替えず、正式窓口へ確認します。承認済み対話AIを利用できるまで、この演習は未完了です。
...
先の人間判断を残したまま、次の架空ケースだけを対話AIへ入力して「結論」ではなく「確認質問」を作らせます。
