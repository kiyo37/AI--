...
| 確認項目 | 内容 |
|---|---|
| 前提 | [01 生成AIの基礎](01_generative_ai_basics.md)から[05 ハルシネーションとファクトチェック](05_hallucination_and_fact_checking.md)までを順に学ぶ |
| 使う収録ファイル | 基本練習は本章内の架空チラシ設定を使う。[架空の業務改善依頼](sample-data/business_interview_initial_request.md)と[保存済みの架空回答](sample-outputs/business_interview_saved_responses.md)は、AI出力のガバナンス上の問題を追加分析するときに使える |
| 成果物 | 非公開のチラシ案、制作記録、公開前リスク記録をまとめた`06_公開前レビュー.md` |
| 開始条件 | 下の2経路から1つを選び、承認・利用条件を確認した対話AIとAIデザイン機能、利用する素材の出所、成果物の非公開保存場所を決めている |
| 停止条件 | 必要なAI環境を利用できない、実在人物・作品・ブランドを使いそう、素材の利用条件が不明、AIが未確認の実績や数字を追加、個別の法的判断が必要 |

### 利用環境を1つ選ぶ

| 経路 | 進め方 |
|---|---|
| 個人学習 | 本人が利用条件、データ設定、素材ライセンスを確認した対話AIやデザインツールを使う。教材の架空設定だけを入力する |
| 組織利用 | 組織が承認したアカウント、ワークスペース、素材だけを使う。共有・ダウンロード・公開は行わない |

> **修了条件**：対話AIによる原稿作成と、Canva AIまたは承認・利用条件を確認した同等のAIデザイン機能による非公開案の作成という**AIの実操作**が必要です。保存済み出力や紙のワイヤーフレームは誤り分析・記録の補助であり、実操作の代わりにはなりません。
...
**図の要点**：実線は制作工程の順番、枠は工程と確認項目の所属関係です。権利確認を公開直前だけに置かず、入力素材をAIへ渡す時点から各工程で行います。「AIが作ったので自由」「人が少し直したので安全」と一括判断せず、重要な商用利用は最新情報と法務・知的財産（知財）の専門家確認へつなぎます。
...
**図の要点**：利用規約と第三者権利は別々に確認します。条項や権利関係が不明なときは推測で進めず、確認日と質問を整理して提供事業者、法務・知財、弁護士等へ確認します。相談した事実だけで通過とせず、利用条件を確認できたかで判断します。

### 3.4 画像生成AIの注意

| リスク | 例 | 対策 |
|---|---|---|
| 既存作品との類似 | 特徴的な構図・キャラクター | 画像検索は手掛かりとして使い、素材・生成条件・権利を別途確認。別案、法務相談 |
...
**権利クリアランス**とは、素材や表現を予定する方法で利用できるか、権利者、ライセンス、契約、第三者の権利等を確認する作業です。類似画像検索で一致が出なかったことは「似た画像を検索で見つけられなかった」という結果にすぎず、権利クリアランスの完了や非侵害を保証しません。また、切り抜き、色変更、反転、文字追加などの人間編集やAI編集を行っても、元素材の権利・契約条件や既存表現との類似が自動的に解消するわけではありません。重要な商用利用は、入力素材、生成過程、出力、利用目的をそろえて法務・知財担当者や専門家へ確認します。
...
教材演習では、実在のキャラクターや有名人を生成させず、架空のブランド設定を使います。

#### 図6-3：画像・人物・キャラクター・ブランド表現の公開判断
...
**図の要点**：「○○風」という言い換えだけで安全になるわけではありません。著作権、商標、肖像、パブリシティ、広告表示など複数の論点があり得るため、実在対象を練習で生成せず、個別案件は専門家へ確認します。
...
**図の要点**：文書だけでなく、責任、技術的統制、教育、監視、改善までがガバナンスです。法令、規約、機能は変わるため、最新版の公式情報と専門家レビューを定期見直しに組み込みます。
...
**図の要点**：全件を一部署へ丸投げせず、判断内容に応じて責任を分けます。最終的な業務利用と対外説明の責任者を明示し、ベンダーやAIそのものを承認者にしません。
...
**要配慮性**は、健康、障害、信条、懲戒歴など、漏えいや不適切な利用によって差別・不利益・深いプライバシー侵害につながりやすく、特に慎重な取扱いが必要かという観点です。法令上の正式な区分や範囲は国・地域で異なるため、自己判断で確定しません。**法的根拠**は、個人情報等を収集・利用・AI処理してよい理由を、適用法令に照らして説明できる根拠です。「業務に便利」「本人が拒否しなかった」「AI利用が社内承認済み」だけで十分とは限りません。目的、通知・同意等の要件、契約、保存期間、本人の権利を個人情報・法務担当者や専門家へ確認します。
...
**図の要点**：全体平均だけで利用可否を決めず、誰にどの不利益が出るかを確認します。採用、雇用、与信、教育評価、医療などの高影響用途ではAIを最終判断者にせず、担当部門と法務・倫理・各領域の専門家による影響評価を行います。
...
**図の要点**：確認者に元資料、時間、専門性、停止権限がなければ、承認印だけの形式的な作業になります。AI出力は必ず人が確認し、権利や法的評価が不明な場合は法務・知財等の専門家へ引き継ぎます。
...
- 組織内で責任分界を合意する検討会
- 対話AIで原稿案、Canva AIまたは同等機能でデザイン案を作り、生成から公開前までのリスクを人間が確認する演習

## 5. つまずきやすい点と確認方法

- **すぐに「合法／違法」と決める**：まず入力素材、生成過程、用途、地域、規約、公開方法という確認事実を列挙します。
- **著作権だけを見て終える**：商標、肖像、パブリシティ、契約、広告表示など、誰のどの利益に影響するかを分けます。
- **古い利用規約を根拠にする**：公式ページ、対象プラン、確認日を記録し、解釈が必要なら法務等へつなぎます。
- **「人が見た」で承認済みにする**：確認項目、担当、証跡、停止権限がそろっているか確認します。

## 6. 手順付き練習

### 演習：対話AIとAIデザイン機能で架空の研修チラシを作り、公開前リスクを確認する

実在する企業、人物、商品、キャラクター、ロゴ、顧客データ、未公開情報は使いません。組織利用で実データ利用が承認されていても、この練習には教材に記載された完全な架空データだけを使います。完成物は自習中だけで見る非公開下書きとし、外部公開、SNS投稿、印刷発注、広告配信は行いません。Canvaの機能名や利用範囲は変わるため、[Canva・Gamma演習環境セットアップ](setup/canva_gamma_setup.md)と最新の公式情報を確認します。

#### 利用環境の準備確認

| 学習環境 | 実施方法 |
|---|---|
| 個人学習 | 本人が条件を確認した対話AIと、Canva AIまたは同等のAIデザイン機能を使います。利用条件、AI・プライバシー設定、素材ライセンスを確認し、完全な架空情報だけを入力します。 |
| 組織利用 | 承認済みの組織アカウントとワークスペースを使い、AI機能、素材、共有、ダウンロードの権限を管理者へ事前確認します。未承認の個人アカウントへ切り替えません。 |

Canva AIまたはMagic Studioの表示名、利用上限、対応言語、利用可能な機能はプラン・地域・管理者設定等で変わります。指定機能が見当たらない場合は、承認・利用条件を確認した同等のAIデザイン機能を使います。利用可能なAIデザイン機能がない場合は、紙上案だけで完了にせず、環境を準備できるまでこの演習を停止します。
...
対話AIには、原稿の事実を増やさず、不足を「要確認」とするよう指示します。

```text
あなたは研修チラシ原稿の編集補助者です。上の架空設定だけを使い、
見出し、短い説明、対象者、当日の内容、注意書きの下書きを作ってください。
実績、推薦、効果、料金、定員、登壇者経歴など、与えられていない事実は作らず「要確認」としてください。
...
生成画像は安全や権利を保証された素材ではありません。似ている対象がないか、固定観念を強める表現がないかを人が確認します。類似画像検索で一致が出ないことや、人が色・形・配置を編集したことも、権利クリアランス完了の証明にはなりません。

#### 手順2：別の架空テーマで制作記録を作る

手順1とは異なる架空テーマで、非公開のチラシ案と公開前確認記録を作ります。

- 対話AIで原稿案を作り、元設定にない実績、推薦、統計、効果を削除する
- Canva AIまたは利用可能な同等機能で、固有の人物・ブランドを含まない画像案を作る
- 使用したプロンプト、生成日時、機能名、選んだ素材、編集内容を記録する
- Canvaライブラリ素材を使う場合は、素材情報と現在のライセンス条件を確認する
- 類似画像検索と編集の結果だけで「権利上問題なし」と判定せず、未確認事項と専門家への質問を残す
...
| 編集 | 未確認の統計・推薦・実績、素材ライセンス。編集前の権利問題が残っていないか | AIが作った数字を根拠なく掲載している、編集しただけで権利確認を完了している |
...
| 自己点検ポイント | 見た目の良さだけで採用せず、第三者の権利、事実、誤認、偏り、共有範囲を確認したか |
| 改善ポイント | 「AI生成だから自由」と判断した場合は入力から公開までを再点検する。判断材料が足りない場合は公開せず、素材差替えまたは専門家相談へ進む |

## 7. 実務チェックリストと振り返り

この節は、生成物を共有・公開する前のセルフレビューとして使います。法律、著作権、個人情報等の結論は地域、用途、契約、事実関係で変わるため、必要に応じて専門家へ確認してください。
...
- [ ] 類似画像検索の一致なし、または人間・AIによる編集を、権利クリアランス完了の保証にしていない
- [ ] 誤認、差別、固定観念、アクセシビリティ上の問題を確認した
- [ ] 個人に関係する情報や表現では、要配慮性と法的根拠を担当者・専門家へ確認した
...
1. `06_公開前レビュー.md`を先に保存します。
2. [第06章の自己点検ガイド](self-check/chapters/06_copyright_and_ai_governance_self_check.md)を開き、段階ヒントだけを先に使います。
