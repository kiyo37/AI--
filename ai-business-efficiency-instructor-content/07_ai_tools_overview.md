...
| 使う収録ファイル | 本章内の架空公開マニュアルM-01を使う。[架空問い合わせケース](sample-data/automation_inquiry_cases.md)と[保存済みテスト出力](sample-outputs/automation_saved_test_output.md)はAI出力の誤り分析を追加で行うときに使える |
| 成果物 | FAQ草案の照合表、利用条件、停止条件、手動代替をまとめた`07_ツール選定記録.md` |
| 開始条件 | 下の2経路から1つを選び、承認・利用条件を確認した対話AI、目的、入力データ、確認者、保存場所を決めている |
| 停止条件 | 対話AIを利用できない、実在情報を入力しそう、料金・試用・共有・データ利用条件が不明、未承認の連携や新規登録が必要 |
...
> **修了条件**：承認・利用条件を確認した対話AIへM-01と完成プロンプトを入力し、FAQ出力を元資料と照合する**AIの実操作**が必要です。保存済み出力の閲覧だけでは本章を完了できません。
...
- 追加比較のために新規登録しません。利用できる承認済み対話AIがない場合は演習を停止し、個人学習では利用条件を確認できる環境、組織利用では組織承認済み環境を準備してから戻ります。[保存済みテスト出力](sample-outputs/automation_saved_test_output.md)は誤り分析の補助に限り、実操作の代わりにはしません。
