...
[資料カード](../../sample-data/rag_question_cards.md)と[保存済み回答](../../sample-outputs/notebooklm_saved_answers.md)を使います。現行版と旧版の原文も必ず横に並べます。

</details>

## 1. 演習の前提

基本練習ではNotebookLMを使えます。Difyは任意の発展学習であり、登録やAPIキーの用意を必須にしません。利用環境は次のいずれかに分けます。

| 状況 | 進め方 |
|---|---|
| 個人学習 | 本人が管理するアカウントと完全な架空資料を使う |
| 組織利用 | 管理者が許可した業務アカウント、機能、ブラウザだけを使う |
| 未承認・利用不可 | 登録・アップロードをせず、[資料カード](../../sample-data/rag_question_cards.md)と[保存済み回答例](../../sample-outputs/notebooklm_saved_answers.md)で進める |
...
[RAG質問・資料カード](../../sample-data/rag_question_cards.md)から質問に関係するカードを選びます。自分で「回答、根拠、適用対象、注意、相談先」の欄を埋めた後、いったん回答を隠して原文と照合します。
...
承認済みのDify環境とモデルがある場合だけ、次の構成を非公開の架空アプリとして設計します。利用できない場合は[Dify紙上設計の保存済み例](../../sample-outputs/dify_saved_design_output.md)を使います。
...
| Difyの登録を必須にする | 契約・認証・費用・データ条件が未確認 | 任意発展とし、APIキー不要の紙上例を用意する |
