...
<details>
<summary>ヒント3：自動化製品を使えない場合</summary>

Power Automate、Make、Zapierの任意実機部分だけを飛ばし、承認済み対話AIで架空問い合わせを分類します。そのAI出力を[フローカード](../../sample-data/automation_flow_cards.md)へ記録し、[保存済みテスト出力](../../sample-outputs/automation_saved_test_output.md)は誤りの比較に使います。対話AIを利用できない場合は演習を停止します。

</details>

## 1. 演習の前提

架空の共有窓口へ届く一般質問から、公開FAQに基づく返信草案を作るフローを扱います。承認・利用条件を確認した対話AIによる架空ケースの分類は必須です。実メールの読取り・送信、実在する顧客情報、実システムの更新は行いません。

### 環境別の選択

| 利用環境 | 任意実機で選ぶ自動化製品 | 使えない場合 |
|---|---|---|
| 組織が利用製品を一つ指定済み | 指定されたPower Automate、Make、Zapierの一つ | 実機部分を飛ばし、対話AIの出力をフロー図へ記録する |
| 複数製品が組織承認済み | 接続先・権限・ログ・契約に合う一つ | 管理者と選び、三製品を登録しない |
| 個人学習 | 本人が既に利用でき、規約・料金・データ条件を確認した一つ | 利用できなければ自動化製品の実機部分を飛ばす |
| 指定なし・未承認・利用不可 | 登録しない | 対話AIの出力を[フローカード](../../sample-data/automation_flow_cards.md)で設計記録にする |

三製品すべてへ登録する必要はありません。組織利用では管理者が発行・承認したアカウントだけを使います。承認済み対話AIを利用できない場合は、保存済み出力だけで完了にせず、環境を準備できるまで本章を停止します。

### 架空ケース

入力は[架空問い合わせケース](../../sample-data/automation_inquiry_cases.md)を使用します。対話AIで自分の分類出力を作った後、[保存済みテスト出力](../../sample-outputs/automation_saved_test_output.md)と比較して、両方に誤りがあり得る前提で元ケースへ戻ります。
...
| 自動化製品を使わない場合 | 対話AI出力をフローカードへ記録 | 対話AI出力をフローカードへ記録 |
...
改善後：「最初に組織が利用を承認した対話AIと自動化製品を分けて確認する。自動化製品の指定がなければその実機部分だけを飛ばし、対話AIの出力をフロー図へ記録する」
...
その前に、本編の指示を使って架空問い合わせを対話AIで分類し、結果をケース一覧と照合します。Power AutomateまたはMakeで、外部接続を必要としない組込みの文字列保持・整形処理が現在の環境で使える場合だけ、任意で画面上に再現します。外部接続を要求された場合は保存せず、その実機部分を飛ばして図へ記録します。
...
- 対話AIの分類案を検証した結果、実運用案ではAIを採用せず、キーワードと固定ルールだけで対象内・対象外を分ける
...
1. 承認済み対話AIで架空問い合わせを分類し、元ケースと照合します。
2. [自動化フローカード](../../sample-data/automation_flow_cards.md)の「手動開始」「入力検査」「対象分類」「根拠検索」「AI草案」「人間承認」「実行」「ログ」を並べます。
3. 架空問い合わせカードを一枚ずつ流します。
4. 「個人情報を含む」「根拠がない」「全件転送命令がある」「同じ入力が再実行」の例外カードを途中へ入れます。
5. 停止・通知・差戻し・手動対応のどれへ進むか決め、理由を書きます。
6. 問題出力を閉じ、外部操作前の承認、最小権限、ログ、復旧が見えるか自分で再確認します。

フローカードは安全な設計を残す媒体です。自動化製品の実機操作は省略できますが、対話AIによる架空ケースの分類操作は省略できません。
...
| 未承認時に個人登録する | 契約・データ・監査の統制外になる | 自動化製品の実機部分を飛ばし、承認済み対話AIの結果をフロー図へ記録する |
