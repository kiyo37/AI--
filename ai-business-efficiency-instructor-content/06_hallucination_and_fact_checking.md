...
### 演習：架空の社内規程回答を検証する（45分）

根拠資料は次の2文だけです。

```text
架空規程A 第3版：在宅勤務申請は原則として希望日の3営業日前までに上長へ提出する。
架空FAQ 2026-06-01版：緊急時は事後申請を認める場合がある。人事部へ相談する。
```

AI回答例：

```text
在宅勤務は全社員が前日18時までにシステム申請すれば自動承認されます。
緊急時も事後申請は禁止です。これは第4版第8条に明記されています。
```

1. 検証可能な主張へ分解します。
2. 各主張を「確認済み・誤り・不明」に分類します。
3. 根拠箇所と不足情報を記録します。
4. 断定を避けた安全な回答案を作ります。

## 7. 確認問題

1. ハルシネーションと意図的な嘘の違いは何ですか。
2. 出典URLが実在するだけでは不十分な理由は何ですか。
3. 最新情報を確認するときに見るべき日付を2つ挙げてください。
4. AIへ同じ質問を再度行うことが独立検証にならない理由は何ですか。
5. 高リスク領域でAIを使える範囲を説明してください。

<details><summary>解答例</summary>

1. ハルシネーションは、事実保証の仕組みではないAIがもっともらしい誤情報を生成する現象で、意図を前提にしません。
2. 本文が主張を支えているか、発行主体、日付、対象、例外が適合するかを確認する必要があるためです。
3. 情報の発行日・更新日と、対象制度・数値の基準日などです。
4. 同じモデルや情報に由来する誤りを繰り返す可能性があるためです。
5. 論点・確認項目・草案の作成までとし、原典確認と有資格専門家の判断を必須にします。
