...
> **位置付け：任意発展**　生成AIの基本、安全な入力、出力確認、業務分解を学んだ後に進む章です。製品への接続や自動実行は必須ではなく、収録済みの架空データによる紙上設計だけで完了できます。実業務のPoCはこの自習と分け、正式な承認と検証環境を整えてから行います。
...
| 開始条件 | 下の3経路から1つを選び、3つの収録ファイルを開ける |
| 停止条件 | 実メール・実カレンダー・実ファイルへ接続しそう、APIキー等を入力しそう、外部書込み前の承認がない、権限や停止方法が不明 |

### 利用環境を1つ選ぶ

| 経路 | 進め方 |
|---|---|
| 個人学習 | 既に利用でき、規約・料金・データ条件を確認した1製品だけを使う。外部サービスへ接続せず、画面上のテストまたは設計だけを行う |
| 組織利用 | 組織が承認した1製品、発行済みアカウント、閉じた検証環境だけを使う。不明なら接続しない |
| AIなし | 架空問い合わせケース、フローカード、保存済みテスト出力を使い、紙上またはMermaidで設計する |
...
本章の練習では3製品すべてを登録しません。[自動化ツールの選択と準備](setup/automation_tools_setup.md)を確認し、最初に組織が学習用として承認・指定した製品があるかを確認します。指定製品がなければ登録せず紙上練習へ進みます。複数製品が承認済みの場合だけ、接続先と統制要件を基に一つ選びます。
...
| 指定なし・未承認・利用不可 | 紙上フロー | 登録や外部接続をせず、トリガー、処理、承認、記録を設計する |
...
[架空問い合わせケース](sample-data/automation_inquiry_cases.md)を使い、架空の共有窓口へ届く問い合わせを対象にします。実在メールや実システムへ送信・書込みをしてはいけません。ツールを使えない場合は[自動化フローカード](sample-data/automation_flow_cards.md)と[保存済みテスト出力](sample-outputs/automation_saved_test_output.md)を使います。
...
| 指定なし・未承認・利用不可 | 登録・接続せず、[フローカード](sample-data/automation_flow_cards.md)またはMermaidで紙上フローを作る |

組織利用では、管理者の承認を登録・接続より先に得ます。実メール、実在の顧客情報、APIキー、パスワードは使用しません。

#### 2. 収録ケースで手順を練習する

1. [架空問い合わせケース](sample-data/automation_inquiry_cases.md)の`FAKE-AUTO-001`を使い、「手動で開始→架空の問い合わせ文を読み込む→分類候補を作る→人へ提示→結果を記録」の流れを作ります。
2. ツールを使える場合も、メール送信や外部サービス接続はせず、画面上のテスト結果または実行履歴で確認します。
3. 紙上の場合は、[自動化フローカード](sample-data/automation_flow_cards.md)を並べ、[保存済みテスト出力](sample-outputs/automation_saved_test_output.md)と比較します。
...
- [ ] 組織が承認・指定した製品を最初に確認し、指定がなければ紙上演習へ切り替えられる
