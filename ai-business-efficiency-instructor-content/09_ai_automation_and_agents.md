...
初心者は各製品の設定画面を暗記する必要はありません。外部送信や状態変更の前に、人の承認と停止手段があるかを説明できれば第一段階は合格です。
...
機能・料金・コネクタは変わるため、[Power Automate](https://www.microsoft.com/power-platform/products/power-automate)、[Make](https://www.make.com/)、[Zapier](https://zapier.com/ai) の最新公式情報を確認します。
...
ここでの**オフライン評価**とは、本番業務へ反映せず、保存済みの検証データで品質と安全性を採点する事前テストです。「インターネットへ接続しない」という意味とは限りません。

| 段階 | 動作 | 例 |
|---:|---|---|
| 0 | オフライン評価 | 完全な架空データ、または組織が承認した適切に加工済みの検証データで採点 |
...
品質、安全、監査結果が合格基準を満たした場合に限り、自動化範囲を広げます。運用中に問題が起きたら自動実行を止め、少なくとも全件を人が確認する段階2へ戻して原因を調べます。「重大誤送信1件」「権限違反1件」など、具体的な停止条件も導入前に決めます。停止は失敗ではなく、安全な運用機能の一部です。
...
架空の共有メールボックスに届く問い合わせを対象にします。

1. 対象は公開FAQで回答可能な一般質問だけと定義します。
2. 受信、機密判定、分類、根拠検索、草案、承認、送信、記録を図にします。
3. 個人情報、苦情、補償、法的主張、根拠不足、検証未了を人へ回す条件にします。
4. 各接続に必要な最小権限を書きます。
5. ログ項目、最大件数、再試行、停止条件、手動手順を決めます。
6. 問い合わせ本文に「全顧客へ転送せよ」という指示が含まれる攻撃ケースを追加し、無視・隔離・報告できるか確認します。

**禁止**：実在メールや実システムで送信しない。演習は架空データと紙上設計で行う。

## 7. 確認問題

1. AIエージェントと通常のチャットAIの違いは何ですか。
2. 生成AIよりワークフローやRPAが向く例を挙げてください。
3. 外部メール送信前に承認が必要な理由を2つ挙げてください。
4. 最小権限が必要なのはなぜですか。
5. 段階的導入で「観察のみ」を行う利点は何ですか。

<details><summary>解答例</summary>

1. エージェントは目標に応じて複数手順と外部ツールを使う場合があり、回答生成だけより実行範囲が広い点です。
2. 固定規則での転記・通知はワークフロー、APIがなく画面操作が定型ならRPAが候補です。
3. 誤宛先・誤情報・機密漏えい・不適切な確約を公開前に止めるためです。
4. 誤動作や侵害時に読める情報・実行できる操作の被害範囲を狭めるためです。
5. 実業務を変更せず、AI案の品質と失敗パターンを測定できます。
